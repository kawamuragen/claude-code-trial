class StockAnalyzer {
    ...
    async fetchStockData(symbol) {
        ...
        try {
            ...
            if (error.message.includes('API call frequency') || error.message.includes('株式コードが見つかりません')) {
                this.useDemoData(symbol);
            } else {
                throw error;
            }
    ...
    async fetchYahooFinanceData(symbol) {
        try {
            ...
            console.log('Yahoo Finance データ構造:', { timestamps: timestamps.length, dailyDataCount: Object.keys(dailyData).length, sampleData: Object.values(dailyData)[0] });
            ...
            this.showMessage(`${symbol}のリアルタイムデータを取得しました（Yahoo Finance）`, 'info');
    ...
    useDemoData(symbol) {
        ...
        const latestPrice = parseFloat(Object.values(demoData)[Object.keys(demoData).length - 1]['4. close']);
        const prevPrice = parseFloat(Object.values(demoData)[Object.keys(demoData).length - 2]['4. close']);
        ...
        this.showMessage(`${symbol}のデモデータを表示しています。実際のデータを取得するにはAlpha Vantage APIキーが必要です。`, 'info');
    }
    ...
    updateUI(symbol, quote, dailyData) {
        ...
        document.getElementById('volume').textContent = volumes[volumes.length - 1]?.toLocaleString() || '--';
    }
    ...
    updateChart(dailyData) {
        ...
        console.log('チャートデータ:', { dates: dates.length, prices: prices.length, samplePrices: prices.slice(0, 5) });
    ...
    calculateTechnicalIndicators(dailyData) {
        ...
        document.getElementById('ma20').textContent = latestMA20 ? `$${latestMA20.toFixed(2)}` : '--';
        document.getElementById('ma50').textContent = latestMA50 ? `$${latestMA50.toFixed(2)}` : '--';
    ...
    showMessage(message, type = 'info') {
        ...
        messageDiv.className = type === 'error' ? 'error-message' : (type === 'info' ? 'info-message' : 'success-message');
